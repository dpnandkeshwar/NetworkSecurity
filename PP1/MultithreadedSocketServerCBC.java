...
class MultithreadedSocketServerCBC extends Thread {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			while (true) {
				...
				BobReflectionAttackCBC sct = new BobReflectionAttackCBC(serverClient, counter); // send the request to a
																							// separate thread
				sct.start();
			}
		} catch (Exception e) {
...
class BobReflectionAttackCBC extends Thread {

	Socket serverClient;
	int clientNo;

	public BobReflectionAttackCBC(Socket inSocket, int counter) {
	...
	public void run() {
		...
		try {
			...
			sharedKeyEncrypt = Cipher.getInstance("DESede/CBC/NoPadding");
			sharedKeyDecrypt = Cipher.getInstance("DESede/CBC/NoPadding");
